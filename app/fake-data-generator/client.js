...
export default function FakeClient(){
  ...
  const generate = () => {
    ...
    setTimeout(()=>{
      ...
      const countryMap = { hi: "भारत", en: "United States", es: "España", fr: "France", de: "Deutschland", ja: "日本", bn: "বাংলাদেশ", ta: "இந்தியா" };
      let arr=[]; for(let i=0;i<count;i++){ arr.push({ id:i+1, name:nList[i%nList.length], email:`user${1000+i}@testmail.com`, phone:`9${Math.floor(100000000+Math.random()*900000000)}`, city:cList[Math.floor(Math.random()*cList.length)], country: countryMap[lang] || "India" }); }
  ...
  const download = (type) => {
    ...
    if(type==="csv"){
      content = BOM + "id,name,email,phone,city,country\n" + data.map(d=>`${d.id},"${d.name.replace(/"/g,'""')}","${d.email}","${d.phone}","${d.city}","${d.country}"`).join("\n");
      mime="text/csv;charset=utf-8";
    }
    if(type==="sql"){
      content = BOM + `-- Fake Data SQL - ${lang}\n` + data.map(d=>`INSERT INTO users (id, name, email, city, country) VALUES (${d.id}, N'${d.name.replace(/'/g,"''")}', '${d.email}', N'${d.city}', N'${d.country}');`).join("\n");
      mime="application/sql;charset=utf-8";
    }
    if(type==="txt"){ content = BOM + data.map(d=>`${d.id}. ${d.name} | ${d.city} | ${d.country} | ${d.email}`).join("\n"); }
