...
function genOne(fields: string[], lang: string){
  ...
  if(fields.includes("UPI ID")) o["UPI ID"] = first.toLowerCase() + "@okpay";
  return o;
}
...
export default function Page(){
  ...
  const getTxt = () => {
    return data.map(function(r){ return Object.entries(r).map(function(kv){ return kv[0] + ": " + kv[1]; }).join(" | "); }).join("\n");
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8f8f7] text-zinc-900">
      <main className="max-w-[1100px] mx-auto px-3 py-4">
        ...
        <p className="text-zinc-500 text-[12px] mt-1">🌐 15 Indian + 15 Global • All Fields Localized</p>
        <div className="bg-white rounded-[24px] border shadow-sm p-5 mt-4">
          <label className="font-bold text-[14px]">🌐 Select Language (All Fields Change)</label>
          ...
          <div className="mt-2 bg-green-100 border text-green-800 text-[12px] font-bold px-3 py-2 rounded-full">Active: {lang} - City, Address, Company bhi {lang} me</div>
        ...
        {data.length>0 && (
          <div className="mt-5 bg-white rounded-[24px] border p-4">
            ...
            <div className="flex gap-2 mt-4"><button onClick={()=>navigator.clipboard.writeText(getStr())} className="flex-1 h-12 rounded-full bg-black text-white font-bold">Copy</button><a href={"data:text/plain;charset=utf-8," + encodeURIComponent(getStr())} download={"fake-data." + (view==="table"?"json":view)} className="flex-1 h-12 rounded-full bg-yellow-400 text-black font-black flex items-center justify-center">Download</a></div>
