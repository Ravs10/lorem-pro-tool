  ...
  return (
    <div className="min-h-screen bg-[#f8f8f7] text-zinc-900">
      <header className="sticky top-0 z-50 bg-black text-white px-4 py-3 flex justify-between items-center">
        ...
        {menuOpen && <div className="absolute top-[48px] left-0 w-full bg-black p-4 flex flex-col gap-2 text-sm"><a href="/">Home</a><a href="/fake-data-generator" className="text-yellow-400 font-bold">Fake Data</a></div>}
      </header>
      <main className="max-w-[1100px] mx-auto px-3 py-4">
        <h1 className="text-[22px] font-black">Fake Data Generator - 15 Languages</h1>
        <div className="bg-white rounded-[20px] border p-4 mt-4">
          <label className="font-bold text-sm">🌐 Select Language (15)</label>
          ...
          <div className="mt-5"><label className="font-bold text-sm">Records: {count} / 1000</label><input type="range" min={1} max={1000} value={count} onChange={e=>setCount(parseInt(e.target.value))} className="w-full mt-2 accent-black" /></div>
        ...
        {data.length>0 && <div className="mt-4 bg-white rounded-[20px] border p-3"><div className="flex gap-2 overflow-auto pb-2">{["table","json","csv","txt","sql"].map(v=><button key={v} onClick={()=>setView(v)} className={`px-4 h-9 rounded-full text-xs font-black uppercase ${view===v?"bg-black text-white":"bg-zinc-100"}`}>{v}</button>)}</div><div className="flex gap-2 mt-3"><button onClick={()=>{localStorage.setItem('saved-data',getStr()); setSaveText("✓ Saved!"); setTimeout(()=>setSaveText("💾 Save"),2000);}} className={`flex-1 h-10 rounded-full text-sm font-bold transition-all ${saveText.includes("Saved")?"bg-green-500 text-white":"bg-zinc-100"}`}>{saveText}</button><button onClick={()=>{navigator.clipboard.writeText(getStr()); setCopyText("✓ Copied!"); setTimeout(()=>setCopyText("Copy"),2000);}} className={`flex-1 h-10 rounded-full text-sm font-bold transition-all ${copyText.includes("Copied")?"bg-green-500 text-white":"bg-black text-white"}`}>{copyText}</button><a href={`data:text/plain;charset=utf-8,[STRIPPED] download={`fake.${view==="table"?"json":view}`} className="flex-1 h-10 rounded-full bg-yellow-400 text-black text-sm font-black flex items-center justify-center">Download</a></div><div className="mt-3 border rounded-xl overflow-auto max-h-[400px]">{view==="table"?<table className="w-full text-[13px]"><thead className="bg-zinc-50 sticky top-0"><tr>{Object.keys(data[0]||{}).map(k=><th key={k} className="text-left p-3 border-b">{k}</th>)}</tr></thead><tbody>{data.map((r,i)=><tr key={i} className="border-t"><td colSpan={20}><div className="flex">{Object.values(r).map((v,j)=><div key={j} className="p-3 min-w-[120px]">{v}</div>)}</div></td></tr>)}</tbody></table>:<pre className="p-3 text-xs whitespace-pre-wrap bg-zinc-50">{getStr()}</pre>}</div></div>}
        <div className="mt-4 bg-white rounded-[20px] border"><button onClick={()=>setShowArticle(!showArticle)} className="w-full p-4 flex justify-between font-black text-left">📘 What is Fake Data? <span>{showArticle?"−":"+"}</span></button>{showArticle && <div className="px-4 pb-4 text-sm text-zinc-600">Fake data is fake but realistic. Now 15 Indian languages support.</div>}</div>
        <div className="mt-4 bg-white rounded-[20px] border p-4"><h3 className="font-black">FAQ</h3><div className="mt-3 space-y-2">{[{q:"Is data real?",a:"No, fake."},{q:"15 languages?",a:"Select dropdown."},{q:"SQL?",a:"SQL tab > Download."}].map((f,i)=><div key={i} className="border rounded-xl overflow-hidden"><button onClick={()=>setFaqOpen(faqOpen===i?null:i)} className="w-full text-left p-3 font-bold flex justify-between text-sm bg-zinc-50">{f.q}<span>{faqOpen===i?"−":"+"}</span></button>{faqOpen===i && <div className="px-3 py-2 text-xs bg-white">{f.a}</div>}</div>)}</div></div>
      </main>
    </div>
  )
}
