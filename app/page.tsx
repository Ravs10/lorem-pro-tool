...
export default function Page(){
  ...
  const getBank=()=>{
    ...
    if(t.includes("politic")||t.includes("राजनीति")||t.includes("चुनाव")) return d.political||d.default;
    if(t.includes("social")||t.includes("समाज")) return d.social||d.default;
    if(t.includes("sport")||t.includes("cricket")||t.includes("खेल")) return d.sports||d.default;
    if(t.includes("educat")||t.includes("शिक्षा")) return d.education||d.default;
    if(t.includes("tech")||t.includes("तकनीक")) return d.tech||d.default;
  ...
  // === 100% FIXED DOWNLOAD FOR ALL LANGUAGES ===
  const downloadTxt=()=>{
    const encoder = new TextEncoder();
    const bom = new Uint8Array([0xEF,0xBB,0xBF]);
    const data = encoder.encode(out);
    const combined = new Uint8Array(bom.length + data.length);
    combined.set(bom,0); combined.set(data,bom.length);
    const blob = new Blob([combined], {type:"text/plain;charset=utf-8"});
    const url = URL.createObjectURL(blob);
    const a=document.createElement("a"); a.href=url; a.download=`lorem-${lang}-${topic}-${count}.txt`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  }
  const downloadHtml=()=>{
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width"><title>Lorem ${lang} ${topic}</title><style>body{font-family:system-ui;padding:20px;line-height:1.8;white-space:pre-wrap} </style></head><body>${out.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\n/g,"<br>")}</body></html>`;
    const blob = new Blob([html], {type:"text/html;charset=utf-8"});
    const url = URL.createObjectURL(blob);
    const a=document.createElement("a"); a.href=url; a.download=`lorem-${lang}-${topic}-${count}.html`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  }
    ...
    <div style={{minHeight:"100vh", background:"linear-gradient(135deg,#667eea 0%,#764ba2 30%,#f093fb 70%,#f5576c 100%)", backgroundAttachment:"fixed"}}>
      ...
      <header style={{background:"rgba(255,255,255,0.95)", backdropFilter:"blur(12px)", position:"sticky", top:0, zIndex:50}}>
        <div style={{maxWidth:"1150px", margin:"0 auto", padding:"14px", display:"flex", alignItems:"center", gap:"12px"}}>
          <div style={{width:"44px", height:"44px", background:"linear-gradient(135deg,#111,#764ba2)", borderRadius:"12px", display:"flex", alignItems:"center", justifyContent:"center", color:"white", fontWeight:900, fontSize:"20px"}}>L</div>
            ...
            <div style={{background:"rgba(255,255,255,0.95)", borderRadius:"20px", padding:"14px", boxShadow:"0 8px 32px rgba(0,0,0,0.15)"}}>
              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px"}}>
                {["para","words","sent","list"].map(k=>(
                  <button key={k} onClick={()=>setType(k)} style={{height:"54px", borderRadius:"12px", border:type===k?"2px solid #111":"1px solid #e2e8f0", background:type===k?"#111":"white", color:type===k?"white":"#111", fontWeight:900, cursor:"pointer"}}>{k.toUpperCase()}</button>
                ))}
              </div>
              <div style={{marginTop:"12px", border:"2px solid #8b5cf6", borderRadius:"14px", padding:"12px", background:"linear-gradient(135deg,#faf5ff,#f3e8ff)", width:"100%", overflow:"hidden"}}>
                <div style={{display:"flex", justifyContent:"space-between"}}><b style={{fontSize:"12px", color:"#111"}}>🤖 AI TOPIC: {aiOn?"ON":"OFF"}</b><button onClick={()=>setAiOn(!aiOn)} style={{background:aiOn?"#7c3aed":"#94a3b8", color:"white", border:"none", padding:"5px 14px", borderRadius:"20px", fontWeight:900, fontSize:"11px"}}>{aiOn?"ON":"OFF"}</button></div>
              ...
              <a href="https://loremipsumpro.blogspot.com" target="_blank" className="shine-btn" style={{display:"block", marginTop:"14px", borderRadius:"12px", padding:"14px", textAlign:"center", textDecoration:"none", color:"white", fontWeight:900, fontSize:"12px"}}>✨ Gujarati | Marathi | Tamil | Telugu | Bengali →</a>
              <div style={{marginTop:"12px", background:"#f1f5f9", borderRadius:"12px", padding:"12px"}}>
              ...
              <button onClick={generateNow} style={{width:"100%", height:"56px", marginTop:"12px", borderRadius:"12px", border:"none", background:"#111", color:"white", fontWeight:900, cursor:"pointer", fontSize:"15px"}}>⚡ GENERATE ({lang} - {count})</button>
            </div>

            <div style={{background:"rgba(255,255,255,0.97)", borderRadius:"20px", border:"1px solid rgba(255,255,255,0.5)", display:"flex", flexDirection:"column", minHeight:"500px", boxShadow:"0 8px 32px rgba(0,0,0,0.15)"}}>
              <div style={{height:"58px", padding:"0 14px", background:"rgba(248,250,252,0.9)", borderBottom:"1px solid #e2e8f0", display:"flex", justifyContent:"space-between", alignItems:"center", borderTopLeftRadius:"20px", borderTopRightRadius:"20px"}}>
                ...
                <div style={{display:"flex", gap:"8px"}}>
                  <button onClick={doCopy} style={{background:copied?"#16a34a":"#111", color:"white", border:"none", padding:"8px 18px", borderRadius:"20px", fontWeight:900, fontSize:"12px", cursor:"pointer", minWidth:"90px"}}>{copied?"✓ COPIED":"COPY"}</button>
                  <button onClick={downloadTxt} style={{background:"white", color:"#111", border:"2px solid #111", padding:"8px 12px", borderRadius:"20px", fontWeight:900, fontSize:"11px", cursor:"pointer"}}>TXT</button>
                  <button onClick={downloadHtml} style={{background:"#7c3aed", color:"white", border:"none", padding:"8px 12px", borderRadius:"20px", fontWeight:900, fontSize:"11px", cursor:"pointer"}}>HTML</button>
                </div>
              </div>
              ...
              <div style={{padding:"12px", background:"rgba(248,250,252,0.8)", borderTop:"1px solid #e2e8f0", borderBottomLeftRadius:"20px", borderBottomRightRadius:"20px", display:"flex", gap:"8px", flexWrap:"wrap"}}>
                <button onClick={doCopy} style={{flex:1, minWidth:"120px", height:"48px", borderRadius:"12px", background:copied?"#16a34a":"#111", color:"white", border:"none", fontWeight:900, cursor:"pointer"}}>{copied?"✓ COPIED!":"COPY TEXT"}</button>
                <button onClick={downloadTxt} style={{flex:1, minWidth:"100px", height:"48px", borderRadius:"12px", background:"white", border:"2px solid #111", fontWeight:900, cursor:"pointer", color:"#111", fontSize:"12px"}}>DOWNLOAD TXT</button>
                <button onClick={downloadHtml} style={{flex:1, minWidth:"120px", height:"48px", borderRadius:"12px", background:"#7c3aed", border:"none", fontWeight:900, cursor:"pointer", color:"white", fontSize:"12px"}}>DOWNLOAD HTML (Hindi Safe)</button>
              </div>
              <div style={{padding:"8px 12px", fontSize:"10px", color:"#64748b", textAlign:"center"}}>TXT me??? aaye to HTML wala download karo - wo 100% sahi khulega</div>
            </div>
          </div>
        ):(
          <div style={{background:"rgba(255,255,255,0.97)", borderRadius:"20px", padding:"24px", color:"#111"}}>
            <h2>{page.toUpperCase()}</h2><p>Contact: support@loremgen.pro</p>
      ...
      <footer style={{background:"rgba(17,17,17,0.95)", color:"#94a3b8", marginTop:"24px", padding:"22px", textAlign:"center", fontSize:"12px"}}>© 2026 LoremGen PRO • <a href="https://loremipsumpro.blogspot.com" target="_blank" style={{color:"#f59e0b"}}>Blogger Version (17 Lang)</a></footer>
    </div>
  )
}
